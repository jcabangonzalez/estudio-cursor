...
target_log_file = "logsec-toolkit/samples/access.log"

print(f"Reading and analyzing log file: {target_log_file}...")
results = analyze_file(target_log_file)
...
if results.get("risk_report"):
    ...
    try:
        print(">> Requesting analysis from Anthropic (Claude 3 Haiku)...")
        anthropic_client = Anthropic(api_key=os.getenv("ANTHROPIC_API_KEY"))
        ai_response = anthropic_client.messages.create(
            model="claude-haiku-4-5-20251001",
            max_tokens=1000,
            messages=[
                {"role": "user", "content": f"Analyze these critical IPs and tell me who to block: {report_text}"}
            ]
        )
        final_report = ai_response.content[0].text
        print(">> Success with Anthropic!\n")

    except Exception as anthropic_error:
        print(f">> Plan A failed: {anthropic_error}")
        print(">> Substitute entering the court. Connecting to Gemini...")
        gemini_client = genai.Client(api_key=os.getenv("GEMINI_API_KEY"))
        gemini_response = gemini_client.models.generate_content(
            model="gemini-2.0-flash",
            contents=f"Analyze these critical IPs and tell me who to block: {report_text}"
        )
        final_report = gemini_response.text
        print(">> Success with Gemini!\n")

    print("=== ARTIFICIAL INTELLIGENCE REPORT ===")
    print(final_report)
else:
    print("\n[!] No critical risks found. AI analysis skipped.")
